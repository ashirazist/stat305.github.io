...
**Example:**
In the heat treating example (example 1in Chapter 1), Loading method was the primary experimental variable. Extraneous variables  **Steel Heat** and **Machining History** were controlled by experimenting on 78 gears from the same heat code, machined as a lot. Then 78 gears were randomly assigned to two groups (one group were laid  and the other were hung from a bar). Without randomization, unintended changes in measurement techniqes would appear in the data as differences between the two loading methods. 
