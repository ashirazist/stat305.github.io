...
= \hat{Var}(b_1)$$
Note that:
>We can get $\hat{Var}(b_0)$ from JMP as $(SE(b_1))^2$
...
&= 2.569648 \pm 2.160\times{(0.0062933)}\\\\
&=(2.5561\ ,\ 2.5833)
\end{align}
...
\begin{align}
&\hat\mu_{Y\vert{x= 5000}} \pm t_{(n-2, 1-\alpha/2)}\ \times \  s_{LF}\sqrt{(\frac{1}{n} + \frac{(x - \overline{x})^2}{\sum(x_i - \overline{x})^2})}\\\\
&= 2.618335 \pm t_{(15-2, 0.975)}\times{(0.005449)}\\\\
&= 2.618335 \pm 2.160\times{(0.005449)}\\\\
&=(2.60656\ ,\ 2.63011)
\end{align}
We are 95% cofident that the true mean density of the ceramics at 5000 psi is between 2.60656 and 2.63011
]
---
layout: true
class: center, middle, inverse
