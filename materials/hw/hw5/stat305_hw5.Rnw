...
\begin{enumerate}
	...
	\item Chapter 4, Exercise 12, page 208  (unless directed otherwise you may use JMP; include plots as requested) [5 pts each part, 20 pts total]
...
<<cache=FALSE,fig.width=5, fig.height=5, out.width='.5\\linewidth', echo=FALSE>>=
  library(ggplot2)
  p<-qplot(weight,failure_time, data = dw,xlab="Weight Exceeding Guidelines (in tonnes)",ylab="Travel Distance to Failure (10,000 miles)") + coord_fixed(.05)
