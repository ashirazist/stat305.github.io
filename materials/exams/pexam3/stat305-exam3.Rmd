...
\begin{itemize}
\item Readings with pH = 11: $`r paste(high_pH,collapse=", ")`$ \\
\item Readings with pH = 3: $`r paste(low_pH,collapse=", ")`$ \\
\end{itemize}
...
$$`r paste(summary_sample, collapse=", ")`$$
Using these observed values, report the following: \vspace{0.5cm}
\qparts{
...
For Compound 1, the first quartile is `r Qfun(cmp1, 0.25)`, the median is is `r Qfun(cmp1, 0.50)` and the interquartile range is `r Qfun(cmp1, 0.75) - Qfun(cmp1, 0.25)`.

For Compound 2, the first quartile is `r Qfun(cmp2, 0.25)`, the median is is `r Qfun(cmp2, 0.50)` and the interquartile range is `r Qfun(cmp2, 0.75) - Qfun(cmp2, 0.25)`.
...
The following plot shows the sale price of `r nrow(dmnds)` diamonds (in thousands of dollars), which were appraised by experts as having an ideal cut, internally flawless clarity, and being colorless or essentially colorless.

```{r fig.width=5, fig.height=5, out.width='.5\\linewidth', fig.cap='Plot depicting the sale price of 375 diamonds with the same quality of cut, clarity, and color. There is a general pattern indicating that higher carat (i.e., the mass) is associated with higher price.', fig.pos="h", fig.align = "center", echo=FALSE}
...
$$\sum_{i=1}^{`r nrows`} x_i = `r sum_x` \hspace{3cm} \sum_{i=1}^{`r nrows`} x_i^2 = `r sum_x_sq`$$

$$\sum_{i=1}^{`r nrows`} y_i = `r sum_y` \hspace{3cm} \sum_{i=1}^{`r nrows`} y_i^2 = `r sum_y_sq`$$

$$\sum_{i=1}^{`r nrows`} x_i y_i = `r sum_xy`$$
...
\part[3] Using the fitted line, what do we suppose the price would be for a `r obstopred[,"x"]` carat diamond? \vspace{3cm}
\part[3] The actual price of a `r obstopred[,"x"]` carat diamond in the data is `r obstopred[,"y"]` thousand dollars. What is the residual for this specific diamond using the linear model? \vspace{3cm}
...
(\verb!price!) and carat (\verb!carat!) and $\text{carat}^2$ (\verb!carat_sq!) as the model variables.
...
\part[3] Using the fitted quadratic relationship, what do we suppose the price would be for a `r obstopred[,"x"]` carat diamond? \vspace{2cm}
\part[3] The actual price of a `r obstopred[,"x"]` carat diamond in the data is `r obstopred[,"y"]` thousand dollars. What is the residual for this specific diamond using the quadratic model? \vspace{2cm}
...
For the outcome where we roll a 5, then $N = 5$ and we will toss the coin 5 times. Using the number of times we flip heads as our outcome of interest, we will have the conditional distribution of $X | N = 5$ to be binomial(5, $0.5$).
